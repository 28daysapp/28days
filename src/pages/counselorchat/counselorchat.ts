...
/**
 * Generated class for the CounselorchatPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-counselorchat',
  templateUrl: 'counselorchat.html',
})
export class CounselorchatPage implements AfterViewChecked {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public chat: ChatProvider,
    ...
    console.log('CounselorchatPage - constructor');
  ...
  ionViewDidLoad() {
    console.log('CounselorchatPage - ionViewDidLoad');
    this.chat.checkstart().then((isstart) => {
      if (isstart) {
        this.gogomessages = [];
        this.gogomsg('상담사에게 채팅을 요청했어!\n확인 후 Push 알림을 줄 거야~!', true).then(() => {
        ...
        this.gogomessages = [
          this.createmsg(true, '상담사에게 채팅을 요청했어!\n확인 후 Push 알림을 줄 거야~!', true),
